#!/usr/bin/env python3
"""Plot the density evolution at a single spatial point as a function of time."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot |psi|^2 vs time at a specific (x, y) point."
    )
    parser.add_argument(
        "-f",
        "--filename",
...
def resolve_psi_file(
    ...
    if not matches:
        ...
        if legacy_file.exists():
            return legacy_file
        raise FileNotFoundError(
            f"No psi file found for index {index} in {sim_dir}."
        )
    if len(matches) > 1:
        raise RuntimeError(
...
def build_default_save_path(
    psi_path: Path, x_coord: float, y_coord: float
) -> Path:
    def sanitize(val: float) -> str:
        return f"{val:+.3f}".replace("-", "m").replace("+", "p").replace(".", "p")

    suffix = f"cut_x{sanitize(x_coord)}_y{sanitize(y_coord)}.png"
...
def main() -> None:
    ...
    fig, ax = plt.subplots()
    ax.plot(times, density_trace)
    ax.set_xlabel("Time")
    ax.set_ylabel(r"$|\psi(x, y, t)|^2$")
    ax.set_title(
        rf"Density at x={x_selected:.3f}, y={y_selected:.3f} ({args.filename})"
    )
    ...
    if save_requested:
        save_path = (
            Path(args.save)
            if args.save
            else build_default_save_path(psi_path, x_selected, y_selected)
        )
        fig.savefig(save_path, dpi=200)

    print(
        f"Extracted density trace at x={x_selected:.4f}, y={y_selected:.4f} "
        f"from {psi_path.name}."
    )
